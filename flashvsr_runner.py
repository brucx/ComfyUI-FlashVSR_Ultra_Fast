...
def prepare_tensors(path: str, dtype=torch.bfloat16):
    if os.path.isdir(path):
        ...
        with Image.open(paths0[0]) as _img0:
            w0, h0 = _img0.size
        frames = [
            torch.from_numpy(np.array(Image.open(p).convert("RGB")).astype(np.float32) / 255.0).to(dtype)
            for p in tqdm(paths0, desc="Loading images")
        ]
    ...
    if is_video(path):
        with imageio.get_reader(path) as rdr:
            ...
            fps = meta.get("fps", 30)
            frames = [
                torch.from_numpy(frame_data.astype(np.float32) / 255.0).to(dtype)
                for frame_data in tqdm(rdr, desc="Loading video frames")
            ]
...
def get_input_params(image_tensor, scale):
    ...
    sW, sH, tW, tH = (
        w0 * scale,
        h0 * scale,
        max(multiple, (w0 * scale // multiple) * multiple),
        max(multiple, (h0 * scale // multiple) * multiple),
    )
    F0 = largest_8n1_leq(N0 + 4)
    if F0 == 0:
...
def input_tensor_generator(image_tensor: torch.Tensor, device, scale: int = 4, dtype=torch.bfloat16):
    ...
    for i in range(Fs):
        ...
        upscaled_tensor = F.interpolate(
            tensor_bchw, size=(h0 * scale, w0 * scale), mode="bicubic", align_corners=False
        )
...
def init_pipeline(model, mode, device, dtype):
    ...
    ckpt_path, vae_path, lq_path, tcd_path = [
    ...
    if mode == "full":
        ...
        pipe.TCDecoder = build_tcdecoder(
            new_channels=[512, 256, 128, 128], device=device, dtype=dtype, new_latent_channels=16 + 768
        )
...
def run_flashvsr_integrated(
    ...
    if not input_path:
        raise gr.Error("Please provide an input video or image folder path!")
    if seed == -1:
    ...
    if _device not in devices and _device != "cpu":
        raise gr.Error(f"Device '{_device}' is not available! Available devices: {devices}")
    if isinstance(_device, str) and _device.startswith("cuda"):
        torch.cuda.set_device(_device)
    if tiled_dit and (tile_overlap > tile_size / 2):
        raise gr.Error("Overlap must be less than half of the tile size!")
    ...
    if tiled_dit:
        ...
        final_output_canvas = torch.zeros((num_aligned_frames, H * scale, W * scale, C), dtype=torch.float32)
        weight_sum_canvas = torch.zeros_like(final_output_canvas)

        for i in tqdm(range(len(tile_coords)), desc="[FlashVSR] Processing tiles"):
            x1, y1, x2, y2 = tile_coords[i]
            input_tile = frames[:, y1:y2, x1:x2, :]
            LQ_tile, th, tw, F0 = prepare_input_tensor(input_tile, _device, scale=scale, dtype=dtype)
        ...
        th, tw, F0 = get_input_params(frames, scale)
